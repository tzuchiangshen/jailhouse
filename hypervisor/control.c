...
enum management_task {CELL_START, CELL_RELOAD, CELL_DESTROY};
...
static bool cell_send_message(struct cell *cell, u32 message,
			      enum msg_type type)
{
	if (cell->config->flags & JAILHOUSE_CELL_NO_COMM_REGION)
		return true;
...
static int cell_start(struct per_cpu *cpu_data, unsigned long id)
{
	...
	if (cell->reloading) {
		/* unmap all reloadable memory regions from the root cell */
		mem = jailhouse_cell_mem_regions(cell->config);
		for (n = 0; n < cell->config->num_memory_regions; n++, mem++)
			if (mem->flags & JAILHOUSE_MEM_RELOADABLE) {
				err = unmap_from_root_cell(mem);
				if (err)
					goto out_resume;
			}
		cell->reloading = false;
	}
...
static int cell_reload(struct per_cpu *cpu_data, unsigned long id)
{
	const struct jailhouse_memory *mem;
	struct cell *cell;
	unsigned int n;
	...
	err = cell_management_prologue(CELL_RELOAD, cpu_data, id, &cell);
	if (err)
		return err;
	...
	/* map all reloadable memory regions into the root cell */
	mem = jailhouse_cell_mem_regions(cell->config);
	for (n = 0; n < cell->config->num_memory_regions; n++, mem++)
		if (mem->flags & JAILHOUSE_MEM_RELOADABLE) {
...
long hypercall(struct per_cpu *cpu_data, unsigned long code, unsigned long arg)
{
	...
	case JAILHOUSE_HC_CELL_RELOAD:
		return cell_reload(cpu_data, arg);
